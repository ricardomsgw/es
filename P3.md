...
In this delivery of the project, we learned to implement the front end of our app. In this implementation it was very important to understand how we can work with vuetify and cypress. In cypress the option to obtain the name of the component or div of the object was useful. In this way, we test the workings of our app. In the case of the CreateTournament funcionality, the tests implemented were based in the creation of a tournament sucessfully and also in two tests we verified what happened if the user wanted create tournaments with number of questions equal zero or without select topics. In another hand, in the case of the AddUser funcionality we tested the behavior of the app when a user wanted to join to an opened tournament. 

Also in this delivery it was important to understand the difference between a tournament with status 'OPENED' and 'CREATED'. Now, the tournaments are created with status 'OPENED' and then when the tournament starts, the tournament will be initiated with status 'STARTED'.
